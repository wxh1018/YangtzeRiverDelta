...
export default {
  ...
  created() {
    ...
    this.$store.commit("setLeftActive", "2");
    setTimeout(() => {
  ...
  methods: {
    ech1(data) {
       ...
       *  @param {所有城市} allcity
       */
      let center = this.allcitys.find((res) => res.市名称 == "南京市");
      this.center = [center["经度"], center["纬度"]];
      ...
      moveLines = data.map((v, id) => {
        ...
        let value = parseInt(v[this.valuename]);
        let lineStyle = {
          width: value / 30000 + 1,
          opacity: 0.9,
          curveness: 0.2,
    ...
    setEch(allData) {
      var option = {
        ...
        geo: this.$setEch.getGeo(100, this.center),
        series: [
          {
            name: "地点",
            type: "scatter",
